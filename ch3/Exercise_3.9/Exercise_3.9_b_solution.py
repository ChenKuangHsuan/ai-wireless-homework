...
# OAMP-Net Task b: gamma_t, theta_t
class OAMPNet_b(nn.Module):
    """
        OAMPNet in Task (b), learnable parameters:
            gamma: Linear residual scaling factors for each iteration.
            theta: Linear correction factors applied to the backward projection matrix.
    """
    def __init__(self, iter_num):
        super(OAMPNet_b, self).__init__()
    ...
    def forward(self, y, H, sigma_w_sq):
        ...
        v_sq = 0.5 # initial variance

        for t in range(self.iter_num):
            # Linear Estimation: Compute LMMSE matrix
            H_trans = H.transpose(1, 2)
            H_Ht = torch.bmm(H, H_trans)
            reg = (sigma_w_sq / v_sq) * torch.eye(N, device=device).unsqueeze(0)  # Regularization term
            mat = H_Ht + reg
            inv_mat = torch.linalg.inv(mat)
            W_LMMSE = torch.bmm(H_trans, inv_mat) # Linear estimator

            # Compute scaled W_t using trace normalization
            W_H = torch.bmm(W_LMMSE, H)  # (batch_size, K, K)
            tr_val = torch.diagonal(W_H, dim1=1, dim2=2).sum(dim=1)
            tr_val = torch.clamp(tr_val, min=1e-10)
            scale = (K / tr_val).view(-1, 1, 1)
            W_t = scale * W_LMMSE   # Final linear estimation matrix

            # Linear residual update with learnable gamma_t
            Hx = torch.bmm(H, x_hat.unsqueeze(-1)).squeeze(-1)
            residual = y - Hx
            W_res = torch.bmm(W_t, residual.unsqueeze(-1)).squeeze(-1)
            r_t = x_hat + self.gamma[t] * W_res

            # Estimate the denoiser variance tau^2 with theta_t
            ...
            # Nonlinear MMSE denoising step
            x_hat_next = self.mmse_denoiser(r_t, tau_sq)

            # Update variance estimate v^2
...
def main():
    print("\nTraining OAMP-Net (task b)...")
    model_b = OAMPNet_b(iter_num)
    model_b = train_model(model_b, snr_db=20)  # train SNR = 20 dB
    ber_results_b = []

    print("\nTesting OAMP-Net (task b)...")
    for snr_db in snr_db_list:
        ber = test_model_ber(model_b, snr_db)
        ber_results_b.append(ber)

    plt.figure(figsize=(10, 6))
    plt.semilogy(snr_db_list, ber_results_b, 's-', label='OAMP-Net-b')
    plt.xlabel('SNR (dB)')
