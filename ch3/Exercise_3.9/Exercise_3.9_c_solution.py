...
class OAMPNet_c(nn.Module):
    ...
    def forward(self, y, H, sigma_w_sq):
        ...
        for t in range(self.iter_num):
            ...
            H_trans = H.transpose(1, 2)  # (batch_size, K, N)
            H_Ht = torch.bmm(H, H_trans)  # (batch_size, N, N)
            reg = (sigma_w_sq / v_sq) * torch.eye(N, device=device).unsqueeze(0)
            mat = H_Ht + reg
            inv_mat = torch.linalg.inv(mat)
            W_LMMSE = torch.bmm(H_trans, inv_mat)  # (batch_size, K, N)
            ...
            # Nonlinear MMSE denoising step with learnalbe phi_t, xi_t
