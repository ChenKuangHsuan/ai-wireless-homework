...
n_output_size = n_RB  # Choose Band
...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, max_power):
        ...
        self.l3 = nn.Linear(250, 120)
        self.l3.weight.data.normal_(0, 0.1)
        self.l4 = nn.Linear(120, action_dim)
        self.l4.weight.data.normal_(0, 0.1)
        self.max_power = max_power
        self.bias_ = 38  # Make the power in [-100, 23]dbm

    def forward(self, x):
        x = F.relu(self.l1(x))
        x = F.relu(self.l2(x))
        x = F.relu(self.l3(x))
        x = self.max_power * torch.tanh(self.l4(x)) - self.bias_
        return x
...
class DDPG_Agent:
    def __init__(self, memory_entry_size):
        ...
        self.target_model = DQN(n_input_size, n_hidden_1, n_hidden_2, n_hidden_3, n_output_size).to(device)
        self.target_model.eval()
        self.optimizer = torch.optim.RMSprop(self.model.parameters(), lr=0.001, momentum=0.05, eps=0.01)

        self.actor = Actor(n_input_size, 1, env.max_power).to(device)
        self.actor_target = Actor(n_input_size, 1, env.max_power).to(device)
        self.actor_target.eval()
        self.actor_target.load_state_dict(self.actor.state_dict())
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=0.0001)

        self.critic = Critic(n_input_size, 1).to(device)
        self.critic_target = Critic(n_input_size, 1).to(device)
    ...
    def predict_band(self, s_t, ep=0.):
        if random.random() > ep:
            with torch.no_grad():
                q_values = self.model(torch.tensor(s_t, dtype=torch.float32).unsqueeze(0).to(device))
                return q_values.max(1)[1].item()
        else:
            return random.choice(range(n_RB))

    def predict_power(self, s_t):
        with torch.no_grad():
            power_value = self.actor(torch.tensor(s_t, dtype=torch.float32).unsqueeze(0).to(device))
            return power_value.item()

    def Q_Learning_mini_batch(self):

        batch_s_t, batch_s_t_plus_1, batch_action_band, batch_action_power, batch_reward = self.memory.sample()

        band = torch.LongTensor(batch_action_band).to(device)
        power = torch.FloatTensor(batch_action_power).unsqueeze(1).to(device)
        ...
        q_acted = q_values.gather(1, band.unsqueeze(1)).squeeze(1)
        ...
        # Update Actor
        a = self.actor_target(state)
        q = self.critic_target(state, a)
        ...
        # Update Critic
        reward1 = reward.unsqueeze(1)
        a_target = self.actor_target(next_state)
        q_tmp = self.critic_target(next_state, a_target)
        # print("a_target:", a_target.size(), "q_tmp", q_tmp.size())
        q_target = reward1 + self.discount * q_tmp
        q_eval = self.critic(state, power)
        # print("reward1:", reward1.size(), "q_target:", q_target.size(), "q_eval:", q_eval.size())
        critic_loss = self.loss_func(q_target.detach(), q_eval)
    ...
    def save_models(self, model_path):
        ...
        torch.save(self.actor.state_dict(), model_path + '_actor.ckpt')
        torch.save(self.actor_target.state_dict(), model_path + '_actor_t.ckpt')
        torch.save(self.critic.state_dict(), model_path + '_critic.ckpt')
        torch.save(self.critic_target.state_dict(), model_path + '_critic_t.ckpt')
    ...
    def load_models(self, model_path):
        ...
        self.actor.load_state_dict(torch.load(model_path + '_actor.ckpt'))
        self.actor_target.load_state_dict(torch.load(model_path + '_actor_t.ckpt'))
        self.critic.load_state_dict(torch.load(model_path + '_critic.ckpt'))
        self.critic_target.load_state_dict(torch.load(model_path + '_critic_t.ckpt'))
...
for ind_agent in range(n_veh * n_neighbor):  # initialize agents
    ...
    agent = DDPG_Agent(memory_entry_size=len(get_state(env)))
    agents.append(agent)
# ----------------------------Training----------------------------------------
    ...
    for i_step in range(n_step_per_episode):
        ...
        for i in range(n_veh):
            for j in range(n_neighbor):
                ...
                action_band = agents[i * n_neighbor + j].predict_band(state, epsi)
                action_power = agents[i * n_neighbor + j].predict_power(state)
                action_band_list.append(action_band)
                action_power_list.append(action_power)

                action_all_band[i, j] = action_band  # chosen RB
                action_all_power[i, j] = action_power  # power level
        ...
        for i in range(n_veh):
            for j in range(n_neighbor):
                ...
                action_band = action_band_list[n_neighbor * i + j]
                action_power = action_power_list[n_neighbor * i + j]
                state_new = get_state(env, [i, j], i_episode / (n_episode - 1), epsi)
                # add entry to this agent's memory
                agents[i * n_neighbor + j].memory.add(state_old, state_new, train_reward, action_band, action_power)

                # training this agent
                if time_step % mini_batch_step == mini_batch_step - 1:
                    # q-learning
                    loss_val_batch, _, _ = agents[i * n_neighbor + j].Q_Learning_mini_batch()
                    #  record_loss[i_episode] = loss_val_batch
                    if i_episode % 50 == 0 and i == 0 and j == 0:
                        print('episode:', i_episode, 'loss:', loss_val_batch, 'reward:', record_reward[i_episode])
