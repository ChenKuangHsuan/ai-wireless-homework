...
class Environ:
    ...
    def Compute_Performance_Reward_Test_sarl(self, action, power):
        """ for random baseline computation """

        actions = action  # the channel_selection_part
        power_selection = power  # power selection
        ...
        power_selection[(np.logical_not(self.active_links_sarl))] = -100
        for i in range(self.n_RB):  # scanning all bands
            indexes = np.argwhere(actions == i)  # find spectrum-sharing V2Vs
            for j in range(len(indexes)):
                receiver_j = self.vehicles[indexes[j, 0]].destinations[indexes[j, 1]]
    ...
    def Compute_Interference_sarl(self, action, power):
        V2V_Interference = np.zeros((len(self.vehicles), self.n_neighbor, self.n_RB)) + self.sig2

        channel_selection = action.copy()
        power_selection = power.copy()
    ...
    def act_for_testing_sarl(self, action, power):

        action_temp = action.copy()
        power_temp = power.copy()
        V2I_Rate, V2V_Rate = self.Compute_Performance_Reward_Test_sarl(action_temp, power_temp)
